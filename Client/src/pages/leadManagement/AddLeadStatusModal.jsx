...
import { createLeadStatus } from '../../api/leadApi';
import { toast } from 'react-toastify';
import CloseIcon from '../../assets/icons/CloseIcon';

const AddLeadStatusModal = ({ isOpen, onClose, onSuccess }) => {
    ...
    const handleSubmit = async () => {
        ...
        setSaving(true);
        try {
            const response = await createLeadStatus(formData);
            
            if (response.status === 201) {
                toast.success(response.data?.[0]?.Message || 'Lead Status created successfully.');
                setFormData({ Name: '', Status: 1 }); 
                setErrors({});
                onSuccess && onSuccess(); 
                onClose && onClose(); 
            } else if (response.status === 200) {
                toast.error(response.data?.[0]?.Message || 'Failed to create lead status.');
            } else {
            ...
            <div className="bg-[#f1f0e9] rounded-lg shadow-lg p-6 w-full max-w-lg h-full flex flex-col">
                ...
                <div className="mt-6 shrink-0 space-y-2">
                    <button
                        ...
                        disabled={saving}
                    >
                        {saving ? 'Creating...' : 'Create Lead Status'}
                    </button>
